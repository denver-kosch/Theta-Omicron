...
const sortedEvents = (table, sort) => {
	if (sort.key) return [...table].sort((a, b) => {
...
const row = (event, navigate) => {
	...
	const statusStyle = (status) => (status === "Pending") ? { backgroundColor: 'yellow', color: 'black' } : (status === "Rejected") ? { backgroundColor: 'red', color: 'black' } : {backgroundColor: '#1AFF00', color: 'black'};
