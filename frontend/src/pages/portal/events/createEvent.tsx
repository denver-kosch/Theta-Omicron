...
import type { CreateEventForm, EventCommitteeOption, EventLocationOption } from "@/types";

const CreateEvent = () => {
    const navigate = useNavigate();
    const defaultValues: Partial<CreateEventForm> = {location: "", committee: "", start: "", end: "", visibility: ""};
    const {register, handleSubmit, watch, formState: {errors}} = useForm<CreateEventForm>({defaultValues});
    const [loaded, setLoaded] = useState(false);
    const [locOptions, setLocOptions] = useState<EventLocationOption[]>([]);
    const [commOptions, setCommOptions] = useState<EventCommitteeOption[]>([]);
    const [officerComms, setOfficerComms] = useState<EventCommitteeOption[]>([]);
    ...
    useEffect(() => {
        const getInfo = async () => {
            const options = await api<{
                ...
                committees: {member: EventCommitteeOption[]; officer: EventCommitteeOption[]};
            }>("eventCreation", {headers: {Authorization: `Bearer ${localStorage.getItem("token")}`}});
            if (!options.success) {
    ...
    const onSubmit = async (data: CreateEventForm) => {
        try {
            ...
            // Append other fields
            Object.entries(otherData).forEach(([key, value]) => formData.append(key, String(value)));

            const newEvent = await api<{newId: string}>("events", {body: formData, headers: {Authorization: `Bearer ${token}`}, method: "POST"});
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="field">
                        ...
                        <select id="visibility" {...register("visibility", {required: true,validate: value => value !== ''})}>
                            ...
                            {commOptions.some(option => option.name === "Pledge Educator") &&
                            <option value="Pledges">Pledges Only</option>
                        }
                            <option value="Committee">Committee Only</option>
                            {commOptions.some(option => option.name === "Executive Committee") &&
