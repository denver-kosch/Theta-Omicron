...
const PortalEvent = () => {
    ...
    const handleRej = async op => {
        if (op === 'del') {
            const res = await api(`events/${id}`, {method: 'DELETE', body: {committeeId: event.committee.id}, headers: token});
    ...
    const handlePend = async op => {
        const res = (op === 'approve') ? 
            await api(`events/${id}/approve`, { committeeId: event.committee.id }, token) :
            await api(`events/${id}/reject`, { reason: rejReason, committeeId: event.committee.id }, token);
        if (!res.success) console.error(res.error);
        navigate(0);
    };
