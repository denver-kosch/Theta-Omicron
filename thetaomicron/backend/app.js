...
app.post("/get-rush", (req, res) => {
  const getRushCommittee = `
    ...
    JOIN Chairmen AS CM USING (memberId) 
...
app.post('/add-member', async (req, res) => {
  ...
  try {
    ...
    db.query(insertMem, [email, hash, fName, lName, status, phone, street, city, state, zip, initiation, graduation], (err, result) => {
      return (err) ? res.status(500).json({ error: 'Failed to add member', details: err }) : res.status(200).json({ success: true, message: 'Member added successfully' });
    });
  } catch (error) {
