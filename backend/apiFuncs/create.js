...
import { ApiError, convertDocumentToPdf } from "../functions.js";
...
export const addEvent = async (req) => {
	...
	try {
		...
		if (req.file) {
			...
			const placeholderImagePath = join(__dirname, 'public','images','events', 'default.jpg');
...
export const uploadMinutes = async (req) => {
	try {
		...
		const outputDir = join(__dirname, 'secure', 'documents', 'minutes');
		...
		else await convertDocumentToPdf({ inputBuffer: req.file.buffer, fileName: `${baseName}${fileExt}`, outputDir });
