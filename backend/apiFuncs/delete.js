...
export const removeEvent = async (req) => {
  ...
  if (!members.includes(_id)) throw new ApiError(401, 'Unauthorized User');
...
export const deleteMinutes = async (req) => {
  ...
  const permissions = await Member.findById(_id, {_id:0, positions:1, privileges:1});
  const committeesAllowed = [ new ObjectId("6658162d3ff76a90701ab5db"), new ObjectId("6658162d3ff76a90701ab5d8") ];
  const allowed = isAdmin || permissions.positions.some(pos => committeesAllowed.some(id => id.toString() === pos.committeeId.toString()));
