...
import { getCommittee, getBros, getBro, getEvents, getCommittees, getLocations, getEventCreation, getEventDetails, getChairmen, getNotes, getPositions, getMinutes, getSelf, getMinutesRecord } from './apiFuncs/read.js';
...
const validatorChecks = {
    login: [ check('email').trim().isEmail(), validateRequest ],
    addMember: [
    ...
    getEvents: [ check('vis').optional().trim(), check('days').optional().isNumeric(), check('mandatory').optional().isBoolean(), validateRequest ],
    updateCommittee: [ check('name').not().isEmpty().isString(), body('rushLink').optional().isURL(), validateRequest ],
    deleteMinutes: [ check('minutesId').not().isEmpty(), validateRequest ]
}

/* ================== Login / Authorization ================== */
app.post('/login', validatorChecks.login, asyncHandler(login));

app.post('/auth', asyncHandler(auth));

/* ================== Create ================== */
app.post('/members',validatorChecks.addMember, asyncHandler(addMember));

app.post('/events', upload.single('image'), validatorChecks.addEvent, asyncHandler(addEvent));
...
app.get("/events", validatorChecks.getEvents, asyncHandler(getEvents));
...
app.put('/committees/:name', validatorChecks.updateCommittee, asyncHandler(updateCommittee));

/* ================== Delete ================== */
app.delete('/events/:id', asyncHandler(removeEvent));

app.delete('/minutes', validatorChecks.deleteMinutes, asyncHandler(deleteMinutes));

/* ================== Miscellaneous ================== */
app.get('/secure/minutes/:filename', getMinutesRecord);